...
if ( ! get_option( 'penci_slider_options' ) ) {
	//option not found, add new
	add_option( 'penci_slider_options', $defaults_options );
}
else {
	$old_opions  = get_option( 'penci_slider_options' );
	$new_options = wp_parse_args( $old_opions, $defaults_options );

	update_option( 'penci_slider_options', $new_options );
}
	...
	foreach( $meta_box['fields'] as $field ){
		...
		switch( $field['type'] ){
			...
			case 'color':
				...
				wp_enqueue_script( 'penci-opts-color-js', PENCI_SLIDER_URL . '/js/field_color.js', array('wp-color-picker'), '1.0', true );
